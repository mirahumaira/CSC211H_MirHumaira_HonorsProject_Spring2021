...
#include "manhattan.h"
#include "ui_manhattan.h"

Manhattan::Manhattan(QWidget *parent) : QDialog(parent, Qt::FramelessWindowHint), ui(new Ui::Manhattan){
    ui->setupUi(this);

    try{
        QPixmap bkgnd(directory.getBoroughBackgroundImageDirectory());    // Setting background image.

        if (bkgnd.isNull()) throw std::runtime_error ("Manhattan.cpp/Manhattan() -- Error: Cannot find image. Check media directory and path.");
    ...
    try{
        QFile csv(directory.getManhattanMasterFileDirectory());         // Reading the lines from Farmers_Market_Bronx
        csv.open(QFile::ReadOnly | QFile::Text);

        if (!csv.isOpen()) throw std::runtime_error("Manhattan.cpp/Manhattan() -- Runtime Error: Error opening master file! Check src directory.");
...
void Manhattan::readCSCRowItems(QTextStream &in){
...
void Manhattan::sortingLocations(){
    ...
    for (int i=0; i < communityDistrict.size(); i++){
        ...
        if (communityDistrictInt[i] == 0){
            std::cerr << "Manhattan Master File -- Error: Wrong input type for community district in file. Check again." << std::endl;      // Input Validation
        }
    }
...
void Manhattan::swap(int& a, int& b){                                              // Swap the ints.
    int temp = a;
    a = b;
    b = temp;
}

void Manhattan::swap(QString& a, QString& b){                                      // Swap the QStrings.
    QString temp = a;
    a = b;
    b = temp;
}

void Manhattan::makeSortedFile(){

    try{
        QFile file(directory.getSortedFileDirectory());                          // Write a Sorted_Farmers_Market file
        file.open(QIODevice::WriteOnly | QIODevice::Text);

        if (!file.isOpen()) throw std::runtime_error("Manhattan.cpp/makeSortedFile() -- Runtime Error: Error opening sorted file! Check src directory.");
...
void Manhattan::setTextBrowserInfo(){
    ...
    try{
        ...
        if (!fileRead.isOpen()) throw std::runtime_error("Manhattan.cpp/setTextBrowserInfo() -- Runtime Error: Error opening sorted file! Check src directory.");
...
void Manhattan::on_pushButton_clicked(){                               // Called when "Next" button is clicked
    ui->DisText1->setText("");
    ...
    try{
        ...
        if (!fileWrite.isOpen()) throw std::runtime_error("Manhattan.cpp/on_pushbutton_clicked() -- Runtime Error: Error opening sorted file! Check src directory.");
...
void Manhattan::mousePressEvent(QMouseEvent *evt){
    oldPos = evt->globalPos();                                                           // Making diagloue frame movable.
}

void Manhattan::mouseMoveEvent(QMouseEvent *evt){
...
void Manhattan::on_pushButton_2_clicked(){
    hide();
}
