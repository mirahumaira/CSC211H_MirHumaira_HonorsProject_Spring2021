...
Bronx::Bronx(QWidget *parent) : QDialog(parent, Qt::FramelessWindowHint), ui(new Ui::Bronx){
    ui->setupUi(this);

    try{
        QPixmap bkgnd(directory.getBoroughBackgroundImageDirectory());    // Setting background image.

        if (bkgnd.isNull()) throw std::runtime_error ("Bronx.cpp/Bronx() -- Error: Cannot find image. Check media directory and path.");
    ...
    try{
        QFile csv(directory.getBronxMasterFileDirectory());             // Reading the lines from Farmers_Market_Bronx
        csv.open(QFile::ReadOnly | QFile::Text);

        if (!csv.isOpen()) throw std::runtime_error("Bronx.cpp/Bronx() -- Runtime Error: Error opening master file! Check files directory.");
...
void Bronx::readCSCRowItems(QTextStream &in){
    ...
    try{
        while (!in.atEnd()){
            ...
            if (sline.isEmpty())  throw  std::runtime_error ( "Bronx.cpp/readCSCRowItems -- Error: Line is empty!");
        ...
        std::cerr << Qexcept.what();
    }
}

void Bronx::sortingLocations(){
    ...
    for (int i=0; i < communityDistrict.size(); i++){
        ...
        if (communityDistrictInt[i] == 0){
            std::cerr << "Bronx Master File -- Error: Wrong input type for community district in file. Check again." << std::endl;      // Input Validation
        }
    }
...
void Bronx::swap(int& a, int& b){                                            // Swap the ints.
    int temp = a;
    a = b;
    b = temp;
}

void Bronx::swap(QString& a, QString& b){                                   // Swap the QStrings.
    QString temp = a;
    a = b;
    b = temp;
}

void Bronx::makeSortedFile(){

    try{
        QFile file(directory.getSortedFileDirectory());                 // Write a Sorted_Farmers_Market file
        file.open(QIODevice::WriteOnly | QIODevice::Text);

        if (!file.isOpen()) throw std::runtime_error("Bronx.cpp/makeSortedFile -- Error: Error opening sorted file! Check files directory.");
...
void Bronx::setTextBrowserInfo(){
    ...
    try{
        ...
        if (!fileRead.isOpen()) throw std::runtime_error("Bronx.cpp/setTextBrowserInfo -- Error: Error opening sorted file! Check file directory.");
        ...
        std::cerr << Qexcept2.what();
    }
}

void Bronx::on_pushButton_clicked(){                               // Called when "Next" button is clicked

    ui->DisText1->setText("");                                     // Clearing the contents of the textbrowsers first.
    ui->NameText1->setText("");
    ui->LocText1->setText("");
    ...
    try{
        ...
        if (!fileWrite.isOpen()) throw std::runtime_error("Bronx.cpp/pushButton_clicked() -- Error: Error opening sorted file! Check files directory.");
        ...
        fileWrite.close();
        setTextBrowserInfo();                                          // Calling setTextBrowserInfo() again as long as user clicks "Next".
    }
...
void Bronx::mousePressEvent(QMouseEvent *evt){
    oldPos = evt->globalPos();                                        // Making diagloue frame movable.
}

void Bronx::mouseMoveEvent(QMouseEvent *evt){
...
void Bronx::on_pushButton_2_clicked(){
   hide();
}
