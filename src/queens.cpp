...
queens::queens(QWidget *parent) : QDialog(parent, Qt::FramelessWindowHint), ui(new Ui::queens){
    ui->setupUi(this);

    try{
        QPixmap bkgnd(directory.getBoroughBackgroundImageDirectory());             // Setting background image.

        if (bkgnd.isNull()) throw std::runtime_error ("Queens.cpp/Queens() -- Error: Cannot find image. Check media directory and path.");
    ...
    try{
        QFile csv(directory.getQueensMasterFileDirectory());                  // Reading the lines from Farmers_Market_Bronx
        csv.open(QFile::ReadOnly | QFile::Text);

        if (!csv.isOpen()) throw std::runtime_error("Queens.cpp/Queens() -- Runtime Error: Error opening master file! Check files directory.");
...
void queens::readCSCRowItems(QTextStream &in){
...
void queens::sortingLocations(){
    ...
    for (int i=0; i < communityDistrict.size(); i++){
        ...
        if (communityDistrictInt[i] == 0){
            std::cerr << "Queens Master File -- Error: Wrong input type for community district in file. Check again." << std::endl;      // Input Validation
        }
    }
...
void queens::swap(int& a, int& b){                            // Swap the ints.
    int temp = a;
    a = b;
    b = temp;
}

void queens::swap(QString& a, QString& b){                   // Swap the QStrings.
    QString temp = a;
    a = b;
    b = temp;
}

void queens::makeSortedFile(){

    try{
        QFile file(directory.getSortedFileDirectory());          // Write a Sorted_Farmers_Market file
        file.open(QIODevice::WriteOnly | QIODevice::Text);

        if (!file.isOpen()) throw std::runtime_error("Queens.cpp/makeSortedFile() -- Runtime Error: Error opening sorted file! Check src directory.");
...
void queens::setTextBrowserInfo(){
    ...
    try{
        ...
        if (!fileRead.isOpen()) throw std::runtime_error("Queens.cpp/setTextBrowserInfo() -- Runtime Error: Error opening sorted file! Check src directory.");
...
void queens::on_pushButton_clicked(){                              // Called when "Next" button is clicked
    ui->DisText1->setText("");
    ui->NameText1->setText("");
    ui->LocText1->setText("");                                     // Clearing the contents of the textbrowsers first.
    ...
    try{
        ...
        if (!fileWrite.isOpen()) throw std::runtime_error("Queens.cpp/on_pushbutton_clicked() -- Runtime Error: Error opening sorted file! Check src directory.");
...
void queens::mouseMoveEvent(QMouseEvent *evt){
...
void queens::on_pushButton_2_clicked(){
    hide();
}
